...
- macOS builds **both x64 and arm64** (see the arch pin under Always-on mode), so a tag produces two DMGs. Installing the CI-built DMG is the reliable way to get a correct desktop build, since CI always runs `npm run build` before packaging (a locally-built app can carry a stale bundled `public/`).
- **Every artifact is named `${productName}-${version}-${os}-${arch}.${ext}`** via the top-level `build.artifactName` — e.g. `PCO Service Dashboard-2.0.0-mac-x64.dmg`, `…-win-x64.exe`. Platform and version are always in the filename so a downloaded installer is self-identifying; without the pin, electron-builder's default omits the arch on the x64 Mac build and the Intel DMG is indistinguishable from a universal one. The workflow's release globs match on extension (`dist/*.dmg`, `dist/*.exe`), so renaming the pattern doesn't break them.
